...
class ClipScorer(torch.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device=device
        self.model = CLIPModel.from_pretrained("/data/user/yluocg/models/clip-vit-large-patch14-local").to(device)
        self.processor = CLIPProcessor.from_pretrained("/data/user/yluocg/models/clip-vit-large-patch14-local")
